<template>
  <div>
    <v-card-title primary-title class="pb-0">
      <h3 class="headline mb-0">{{infoTab.name}}</h3>
      <v-chip v-if="checkQuantity" color="red lighten-3" small>QTY: {{infoTab.currentStock}}</v-chip>
      <v-chip v-else color="blue lighten-3" small>QTY: {{infoTab.currentStock}}</v-chip>
    </v-card-title>
    <v-subheader>{{infoTab.assay}} - {{infoTab.vendor}} - Catalog #{{infoTab.catalogNumber}}</v-subheader>
    <v-card-text>
      <v-container fill-height grid-list-md>
        <v-card flat color="transparent" class="box">
          <v-card-text class="pa-0" v-if="infoTab.itemDescription">Description: {{infoTab.itemDescription}}</v-card-text>
...
export default {
  computed: {
    ...
    checkQuantity () {
      return this.infoTab.currentStock < this.infoTab.reorderPoint
    }
  }
}
</script>
